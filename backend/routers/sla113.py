...
class CreateSpriteEntryRequest(BaseModel):
    id: str
    name: str
    role: str = "Boss"
    ...
    def validate_id(cls, v: str) -> str:
        normalized = re.sub(r"[^A-Z0-9_]", "_", (v or "").strip().upper())
        if not normalized:
            raise ValueError("Sprite id is required")
    ...
    def validate_name(cls, v: str) -> str:
        name = (v or "").strip()
        if not name:
            raise ValueError("Sprite name is required")
        return name[:120]


DEFAULT_SPRITE_REGISTRY = [
    {
        "id": "LOBO_NEGRO",
        "name": "Lobo Negro",
        "role": "Boss",
        "status": "registered",
        "sheet_url": "https://customer-assets.emergentagent.com/job_3653cf8a-8710-488d-846f-2f0428b714dd/artifacts/g7h5sjnl_spritesheet1%20%281%29.jpg",
        "frame_width": 256,
        "frame_height": 256,
        "frames": 16,
    },
...
# ─── Sprite Registry API ───
@router.get("/sprites")
async def list_sprite_entries():
    """List all sprite registry entries with seeded defaults."""
    cursor = sprite_registry_collection().find({}, {"_id": 0}).sort("created_at", -1)
    entries = await cursor.to_list(200)
    defaults_by_id = {entry["id"]: entry for entry in DEFAULT_SPRITE_REGISTRY}
    custom_by_id = {entry["id"]: entry for entry in entries}
    merged = [custom_by_id.get(sprite_id, defaults_by_id[sprite_id]) for sprite_id in defaults_by_id]
    for entry in entries:
        if entry["id"] not in defaults_by_id:
            merged.append(entry)
    return {"sprites": merged, "total": len(merged)}


@router.post("/sprites")
async def create_sprite_entry(req: CreateSpriteEntryRequest):
    """Create or overwrite a sprite registry entry."""
    now = datetime.now(timezone.utc).isoformat()
    entry = {
        "id": req.id,
        "name": req.name,
        ...
        "frame_width": max(1, req.frame_width),
        "frame_height": max(1, req.frame_height),
        "frames": max(1, req.frames),
        "created_at": now,
        "updated_at": now,
    }
    await sprite_registry_collection().update_one({"id": req.id}, {"$set": entry}, upsert=True)
    return entry


@router.delete("/sprites/{sprite_id}")
async def delete_sprite_entry(sprite_id: str):
    """Delete a custom sprite entry by id."""
    normalized = re.sub(r"[^A-Z0-9_]", "_", (sprite_id or "").strip().upper())
    if not normalized:
        raise HTTPException(status_code=400, detail="Invalid sprite id")
    result = await sprite_registry_collection().delete_one({"id": normalized})
    if result.deleted_count == 0:
        raise HTTPException(status_code=404, detail="Sprite entry not found")
    return {"deleted": True, "id": normalized}
